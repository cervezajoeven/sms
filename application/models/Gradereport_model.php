...
class Gradereport_model extends CI_Model
{
    ...
    // public function get_class_record_quarterly($_school_year, $_grade_level, $_section, $subject, $teacher)
    {
        $quarter_columns = "";        
        $average_columns = "";
        $average_column = "";
        ...
        $main_sql = "SELECT *, ((~first_sum~)/~subject_count~) as q1_ave, ((~second_sum~)/~subject_count~) as q2_ave, ((~third_sum~)/~subject_count~) as q3_ave, ((~fourth_sum~)/~subject_count~) as q4_ave FROM ";
        ...
        foreach($subjects as $row) 
        {
            ...
            $quarter_columns = "";
            foreach($quarters as $qtrrow) 
            {
                if (!empty($quarter_columns)) 
                {
                    $quarter_columns .= ", IFNULL(tbl".$qtrrow->id.".quarterly_grade, 0) AS ".$qtrrow->name."_".$subject_counter;
                }
                else {
                    $quarter_columns .= " IFNULL(tbl".$qtrrow->id.".quarterly_grade, 0) AS ".$qtrrow->name."_".$subject_counter;
                }
            ...
            {
                // $first_sum .= $table_alias . ".Q1_".$subject_counter;
                // $second_sum .= $table_alias . ".Q2_".$subject_counter;
                // $third_sum .= $table_alias . ".Q3_".$subject_counter;
                // $fourth_sum .= $table_alias . ".Q4_".$subject_counter;
        ...
        // print_r(json_encode($quarter_sum));die();
        ...
        // $main_sql = str_replace("~first_sum~", $first_sum, $main_sql);
        // $main_sql = str_replace("~second_sum~", $second_sum, $main_sql);
        // $main_sql = str_replace("~third_sum~", $third_sum, $main_sql);
        // $main_sql = str_replace("~fourth_sum~", $fourth_sum, $main_sql);
